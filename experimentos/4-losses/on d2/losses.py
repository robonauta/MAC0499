...
def generalized_dice_loss(y_true, y_pred):
    ...
    wc = 1/(K.pow(K.sum(y_true, axis=(0, 1, 2)), 2)+K.epsilon())
    num = wc*K.sum(y_true*y_pred, axis=(0, 1, 2))
    dem = wc*K.sum(y_true+y_pred, axis=(0, 1, 2))
    gdl = 1-2*(K.sum(num)+K.epsilon())/(K.sum(dem)+K.epsilon())
